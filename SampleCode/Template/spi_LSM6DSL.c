...
uint8_t appLSM6DSL_Read_Reg(uint8_t addr)
{
	uint8_t rx = 0;

	SPI_SET_SS_LOW(SPI0);

	SPI_WRITE_TX(SPI0, addr | 0x80);
...
void appLSM6DSL_Write_Reg(uint8_t addr , uint8_t value) 
{
	SPI_SET_SS_LOW(SPI0);
...
{
	UNUSED(DeviceAddr);		
	UNUSED(NumByteToRead);	
	* pBuffer = appLSM6DSL_Read_Reg(RegisterAddr);
}
                               ...
                               uint8_t* pBuffer)
{
	...
	UNUSED(DeviceAddr);		
	UNUSED(NumByteToWrite);
	appLSM6DSL_Write_Reg(RegisterAddr ,*pBuffer);
}
